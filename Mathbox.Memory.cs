        ...
        class Memory
        {
            ...
            public Memory()
            {
                try
                {
                    ...
                    ROM low = ROM.LoadEither("136029.103", "136029-103.bin", 0x2000, 0x6A797);
                    ROM high = ROM.LoadEither("136029.104", "136029-104.bin", 0x2000, 0x43382);
                    for (int s = 0; s < 0x2000; s++)
                        Data[address++] = (UInt16)(high[s] * 256 + low[s]);

                    low = ROM.LoadEither("136029.101", "136029-101.bin", 0x4000, 0x150247);
                    high = ROM.LoadEither("136029.102", "136029-102.bin", 0x4000, 0xF557F);
